...
thread_set_nice (int new_nice) 
{
  ...
  calculate_priority_mlfqs(curr);
  
  if(get_max_priority_ready() > curr->curr_priority)
  {
    thread_yield();
  }
}
...
{
  /* Not yet implemented. */
  return convert_fp_to_int_round_near(load_avg * 100);
}
...
{
  /* Not yet implemented. */
  return convert_fp_to_int_round_near((thread_current()->recent_cpu) * 100);
}

...
{
  return ( list_entry(e1, struct thread, elem)->curr_priority > (list_entry(e2, struct thread, elem)->curr_priority));
}
/* Offset of `stack' member within `struct thread'.
   Used by switch.S, which can't figure it out on its own. */

void calculate_priority_mlfqs(struct thread* thread)
{
  if(thread == idle_thread) return;
  int temp_priority = thread->curr_priority;
  int fp_recent = thread->recent_cpu;
  // 계산
  temp_priority = convert_fp_to_int_round_near(subtract_fp(subtract_fp(
    convert_int_to_fp(PRI_MAX), divide_fp_int(fp_recent, 4)),
                    multiply_fp_int(convert_int_to_fp(thread->nice), 2)));

  if(temp_priority > PRI_MAX) temp_priority = PRI_MAX;
  if(temp_priority < PRI_MIN) temp_priority = PRI_MIN;
  thread->curr_priority = temp_priority;
}

void calculate_recent_cpu_mlfqs(struct thread* thread)
{
  if(thread == idle_thread) return;
  int fp_recent = thread->recent_cpu;
  int fp_load_avg = load_avg;
  
  int mla = multiply_fp_int(fp_load_avg, 2);
  int mlaa = add_fp(mla, convert_int_to_fp(1));
  int fpnice = convert_int_to_fp(thread->nice);
  /* printf("l_avg : %d.%02d, mla : %d, mlaa : %d, fpnice : %d, fp_recent: %d\n", convert_fp_to_int_round_near(load_avg)*100, convert_fp_to_int_round_near(load_avg),
  convert_fp_to_int_round_near(mla), convert_fp_to_int_round_near(mlaa), convert_fp_to_int_round_near(fpnice),
  convert_fp_to_int_round_near(fp_recent));
   */
  fp_recent = add_fp(multiply_fp(divide_fp(mla, mlaa), fp_recent), fpnice);
  
  thread->recent_cpu = fp_recent;
}

void calculate_load_avg_mlfqs(void)
{
  int temp_la = load_avg;
  int idle_param = 0;
  if(thread_current() != idle_thread) idle_param = 1;

  load_avg = add_fp(
    multiply_fp(divide_fp_int(convert_int_to_fp(59), 60), temp_la),
    multiply_fp_int(divide_fp_int(convert_int_to_fp(1), 60), list_size(&ready_list)+idle_param));
  
  if(load_avg < LOAD_AVG_NORMAL) load_avg = LOAD_AVG_NORMAL;
}
...
{
  struct list_elem* all_elem = list_front(&all_list);
  struct thread* temp_thread;
  for(; all_elem != list_end(&all_list); all_elem = list_next(all_elem))
  {
    temp_thread = list_entry(all_elem, struct thread, allelem);
    calculate_recent_cpu_mlfqs(temp_thread);
    calculate_priority_mlfqs(temp_thread);
  }
}

/* guswo 스레드의 recent_cpu를 1 증가시킨다*/
void increase_recent_cpu(void)
{
  struct thread* thr = thread_current();
  int fp_recent = thr->recent_cpu;
